...
public final class PlayerSession {
	...
	public PlayerSession(@NotNull BedrockSession bedrockSession, @NotNull TcpClientSession javaSession,
			@NotNull PlayerConnectorPluginWrapper playerConnectorPluginWrapper,
			@NotNull Consumer<PlayerSession> disconnectCallback) {
		...
		this.inventoryManager = new InventoryManager(this, this.fabricRegistryManager,
				(MinecraftCodecHelper) this.java.getCodecHelper(), this.playerConnectorPluginWrapper);
	...
	public void disconnect(boolean fromServer) {
		...
		try {
			disconnectResponse = this.playerConnectorPluginWrapper
					.onPlayerDisconnected(this.javaPlayerEntityInstance.getHealth());
	...
	public void onJavaLogin(@NotNull ClientboundLoginPacket clientboundLoginPacket) {
		...
		this.javaPlayerEntityInstance = this.entityManager.registerLocalPlayerEntity(javaPlayerEntityId,
		...
		// startGamePacket.getGamerules().add(new GameRuleData<>("dodaylightcycle",
		// false));
		...
		startGamePacket.setPlayerGameType(
				GameType.valueOf(clientboundLoginPacket.getCommonPlayerSpawnInfo().getGameMode().name()));
		...
		startGamePacket.setLevelGameType(
				GameType.valueOf(clientboundLoginPacket.getCommonPlayerSpawnInfo().getGameMode().name()));
		startGamePacket.setDifficulty(1);
		startGamePacket.setDefaultSpawn(Vector3i.ZERO);
		startGamePacket.setAchievementsDisabled(true);
		// startGamePacket.setDayCycleStopTime(6000); // TODO: set according to
		// buildplate day/night mode
		...
		genoaGameplaySettingsPacket
				.setOwnerRuntimeId(this.javaPlayerEntityInstance.bedrockEntityInstance.getInstanceId());
		...
		availableEntityIdentifiersPacket.setIdentifiers(NbtMap.builder()
				.putList("idlist", NbtType.COMPOUND,
						Arrays.stream(EarthEntitiesRegistry.getEntities()).map(entityInfo -> NbtMap.builder()
		...
		ServerboundClientInformationPacket serverboundClientInformationPacket = new ServerboundClientInformationPacket(
				"en_GB", HARDCODED_CHUNK_RADIUS, ChatVisibility.FULL, true, Arrays.asList(SkinPart.values()),
		...
		ServerboundCustomPayloadPacket serverboundCustomPayloadPacket = new ServerboundCustomPayloadPacket(
				Key.key("fountain", "earth_mode"), data);
		this.sendJavaPacket(serverboundCustomPayloadPacket);

		this.inventoryManager.initialiseServerInventory();

		ServerboundSetCarriedItemPacket serverboundSetCarriedItemPacket = new ServerboundSetCarriedItemPacket(
				this.bedrockSelectedHotbarSlot);
	...
	public void loadJavaBiomes(List<RegistryEntry> biomesList) {
		this.javaBiomes.clear();
		for (int i = 0; i < biomesList.size(); i++) {
			RegistryEntry entry = biomesList.get(i);
			int id = i;
	...
	public void onJavaLevelChunk(@NotNull ClientboundLevelChunkWithLightPacket clientboundLevelChunkWithLightPacket) {
		this.chunkManager.onJavaLevelChunk(clientboundLevelChunkWithLightPacket, this.javaBiomes,
				(MinecraftCodecHelper) this.java.getCodecHelper());
	}
	...
	public void onJavaEntityAdd(@NotNull ClientboundAddEntityCustomPacket clientboundAddEntityPacket) {
		...
		if (clientboundAddEntityPacket.getType() != null) {
			entityInstance = EntityTranslator.createEntityInstance(clientboundAddEntityPacket.getType(),
					clientboundAddEntityPacket.getData(), this.fabricRegistryManager);
			if (entityInstance == null) {
				LogManager.getLogger().warn("Ignoring Java entity with type {}",
						clientboundAddEntityPacket.getType().name());
				return;
			}
		} else {
		...
		entityInstance.setInitialPosition(
				Vector3f.from(clientboundAddEntityPacket.getX(), clientboundAddEntityPacket.getY(),
						clientboundAddEntityPacket.getZ()),
				Vector3f.from(clientboundAddEntityPacket.getMotionX(), clientboundAddEntityPacket.getMotionY(),
						clientboundAddEntityPacket.getMotionZ()),
				true,
	...
	public void onJavaEntityMove(int javaEntityInstanceId, @Nullable Vector3f pos, @Nullable Vector2f rot,
			boolean onGround, boolean relative) {
		if (javaEntityInstanceId == this.javaPlayerEntityInstance.getInstanceId()) {
	...
	public void onJavaEntityEvent(int javaEntityInstanceId, @NotNull EntityEvent entityEvent) {
		...
		if (entityInstance != null) {
			if (!entityInstance.handleEvent(entityEvent)) {
				LogManager.getLogger().debug("Entity event with type {} ignored by entity ID {}", entityEvent.name(),
						javaEntityInstanceId);
			}
		}
	}
	...
	public void onJavaEntityAnimation(int javaEntityInstanceId, @NotNull Animation animation) {
		...
		if (entityInstance != null) {
			if (!entityInstance.handleAnimation(animation)) {
				LogManager.getLogger().debug("Entity animation with type {} ignored by entity ID {}", animation.name(),
						javaEntityInstanceId);
			}
		}
	}
	...
	public void onJavaEntitySetPassengers(int parentJavaEntityInstanceId, int[] passengerJavaEntityInstanceIds) {
		...
		EntityManager.JavaEntityInstance[] passengerEntityInstances = Arrays.stream(passengerJavaEntityInstanceIds)
				.mapToObj(entityId -> {
	...
	public void onJavaEntityTaken(@NotNull ClientboundTakeItemEntityPacket clientboundTakeItemEntityPacket) {
		...
		EntityManager.JavaEntityInstance collectedJavaEntityInstance = this.entityManager
				.getJavaEntity(collectedJavaEntityId);
		...
		if (collectorJavaEntityId == this.javaPlayerEntityInstance.getInstanceId()) // TODO: handle items taken by other
																					// players
		{
			ItemData item = ((ItemJavaEntityInstance) collectedJavaEntityInstance).getItem();
			if (item != null) {
				this.sendItemParticle(item.getDefinition().getRuntimeId(), item.getDamage(),
						collectedJavaEntityInstance.getPos(),
						this.javaPlayerEntityInstance.bedrockEntityInstance.getInstanceId());
			}
		} else {
	...
	public void onJavaLevelEvent(@NotNull Vector3i position, @NotNull LevelEvent event,
			@NotNull LevelEventData eventData) {
		if (!this.effectManager.handleLevelEvent(position, event, eventData)) {
			LogManager.getLogger().debug("Unhandled level event {}",
					event instanceof LevelEventType ? ((LevelEventType) event).name() : event.getId());
		}
	}

	public void onJavaParticleEvent(@NotNull Particle particle, @NotNull Vector3f position, @NotNull Vector4f offset,
	...
	private void handlePlayerDead() {
		...
		if (respawn) {
			ServerboundClientCommandPacket serverboundClientCommandPacket = new ServerboundClientCommandPacket(
					ClientCommand.RESPAWN);
			this.sendJavaPacket(serverboundClientCommandPacket);

			// TODO: immediately move the player back to the last client-sent position
			// rather than waiting for the client to send the next position update
		} else {
	...
	public void playerInteraction(@NotNull InventoryTransactionPacket packet) {
		if (packet.getTransactionType() == InventoryTransactionType.ITEM_USE) {
			...
			{
				ServerboundUseItemOnPacket serverboundUseItemOnPacket = new ServerboundUseItemOnPacket(
						packet.getBlockPosition(), Direction.VALUES[packet.getBlockFace()], Hand.MAIN_HAND,
						packet.getClickPosition().getX(), packet.getClickPosition().getY(),
						packet.getClickPosition().getZ(), false, 0);
			...
			{
				ServerboundPlayerActionPacket serverboundPlayerActionPacket = new ServerboundPlayerActionPacket(
						PlayerAction.START_DIGGING, packet.getBlockPosition(), Direction.VALUES[packet.getBlockFace()],
						0);
			...
			EntityManager.BedrockEntityInstance entityInstance = this.entityManager
					.getBedrockEntity(packet.getRuntimeEntityId());
			if (entityInstance == null) {
				LogManager.getLogger().warn("Client tried to interact with entity ID {} that does not exist",
	...
	public void playerBreakBlock(@NotNull Vector3i position, int face) {
		ServerboundPlayerActionPacket serverboundPlayerActionPacket = new ServerboundPlayerActionPacket(
				PlayerAction.START_DIGGING, position, Direction.VALUES[face], 0);
		this.sendJavaPacket(serverboundPlayerActionPacket);
	}
	...
	public void playerItemPickup(long itemEntityId) {
		...
		if (bedrockEntityInstance == null
				|| !(bedrockEntityInstance instanceof ItemJavaEntityInstance.ItemBedrockEntityInstance)) {
			LogManager.getLogger().warn(
					"Client sent item pickup with entity ID {} that does not exist or is not an item entity",
					itemEntityId);
			return;
		}
		ItemJavaEntityInstance javaEntityInstance = ((ItemJavaEntityInstance.ItemBedrockEntityInstance) bedrockEntityInstance)
				.getJavaEntityInstance();
		javaEntityInstance.sendPickup();
	}

	public void updateSelectedHotbarItem(@NotNull MobEquipmentPacket mobEquipmentPacket) {
		if (mobEquipmentPacket.getRuntimeEntityId() != this.javaPlayerEntityInstance.bedrockEntityInstance
				.getInstanceId()) {
			...
			LogManager.getLogger().warn("Unrecognised MobEquipmentPacket (unrecognised container ID {})",
		...
		if (mobEquipmentPacket.getContainerId() == 0) {
			...
			this.inventoryManager.sendClientHotbar(); // required because otherwise the client sometimes displays an old
														// item in the selected slot
		} else if (mobEquipmentPacket.getContainerId() == 125) {
			// switch between interact and break/pickup mode
			// we use the last two hotbar slots to represent these on the Java server, as
	...
	public void onJavaItemPickupParticle(byte[] data) {
		try {
			...
			if (bedrockMapping == null) {
				LogManager.getLogger().warn("Item pickup particle for item with no mapping {}",
						JavaItems.getName(javaId, this.fabricRegistryManager));
				return;
			}

			this.sendItemParticle(bedrockMapping.id, bedrockMapping.aux, pos.toFloat(),
					this.javaPlayerEntityInstance.bedrockEntityInstance.getInstanceId());
	...
	public void onJavaSetCarriedItem(int slot) {
		if (slot != this.bedrockSelectedHotbarSlot) {
			LogManager.getLogger().warn("Server set selected hotbar slot to {}, client has requested {}", slot,
					this.bedrockSelectedHotbarSlot);
	...
	public void onJavaContainerSetContent(
			@NotNull ClientboundContainerSetContentPacket clientboundContainerSetContentPacket) {
	...
	public void sendItemParticle(int bedrockItemId, int bedrockItemDataValue, @NotNull Vector3f fromPosition,
			long pickedUpByRuntimeEntityId) {
