...
public class ChunkManager {
	...
	public void onJavaLevelChunk(@NotNull ClientboundLevelChunkWithLightPacket clientboundLevelChunkWithLightPacket, @NotNull HashMap<Integer, String> javaBiomesMap, @NotNull MinecraftCodecHelper minecraftCodecHelper) {
		// TODO: put biome map and codec helper in class fields rather than taking them as parameters every time
		...
		try {
			...
			{
				ChunkSection chunkSection = minecraftCodecHelper.readChunkSection(byteBuf);
	...
	public void onJavaBlockEvent(@NotNull ClientboundBlockEventPacket clientboundBlockEventPacket) {
		...
		if (blockValue instanceof NoteBlockValue) {
			...
			if (bedrockBlockEntityData != null) {
				...
				case PUSHING -> bedrockBlockEntityData.toBuilder().putByte("State", (byte) 0).putByte("NewState", (byte) 1).putFloat("Progress", 0.0f).putFloat("LastProgress", 0.0f).build();
				case PULLING -> bedrockBlockEntityData.toBuilder().putByte("State", (byte) 2).putByte("NewState", (byte) 3).putFloat("Progress", 1.0f).putFloat("LastProgress", 1.0f).build();
				case CANCELLED_MID_PUSH -> bedrockBlockEntityData.toBuilder().putByte("State", (byte) 1).putByte("NewState", (byte) 3).putFloat("Progress", bedrockBlockEntityData.getFloat("Progress")).putFloat("LastProgress", bedrockBlockEntityData.getFloat("Progress")).build();
				};
	...
	public void tick() {
		Arrays.stream(this.chunks).flatMap(Arrays::stream).forEach(chunk -> {
			Arrays.stream(chunk.getBlockEntities()).forEach(nbtMap -> {
				...
				case 0 -> nbtMap.toBuilder().putByte("State", newState).putFloat("Progress", 0.0f).putFloat("LastProgress", 0.0f).build();
				case 2 -> nbtMap.toBuilder().putFloat("State", newState).putFloat("Progress", 1.0f).putFloat("LastProgress", 1.0f).build();
				case 1 -> nbtMap.toBuilder().putByte("State", newState).putByte("NewState", nbtMap.getFloat("Progress") == 0.5f ? (byte) 2 : newState).putFloat("Progress", nbtMap.getFloat("Progress") == 0.5f ? 1.0f : 0.5f).putFloat("LastProgress", nbtMap.getFloat("Progress")).build();
				case 3 -> nbtMap.toBuilder().putByte("State", newState).putByte("NewState", nbtMap.getFloat("Progress") == 0.5f ? (byte) 0 : newState).putFloat("Progress", nbtMap.getFloat("Progress") == 0.5f ? 0.0f : 0.5f).putFloat("LastProgress", nbtMap.getFloat("Progress")).build();
				default -> nbtMap;
				};
