...
public final class GenoaInventoryHelper {
	...
	public GenoaInventoryHelper(@NotNull FabricRegistryManager fabricRegistryManager,
	...
	public Inventory getInventoryFromConnectorPlugin() {
		...
		for (micheal65536.fountain.connector.plugin.Inventory.StackableItem stackableItem : connectorPluginInventory
				.getStackableItems()) {
			inventory.stackableItems.put(stackableItem.uuid, stackableItem.count);
		}
		for (micheal65536.fountain.connector.plugin.Inventory.NonStackableItem nonStackableItem : connectorPluginInventory
				.getNonStackableItems()) {
			inventory.nonStackableItems.computeIfAbsent(nonStackableItem.uuid, uuid -> new HashMap<>())
					.put(nonStackableItem.instanceId, nonStackableItem.wear);
		}
		...
		for (int index = 0; index < 7; index++) {
			...
			if (hotbarItem == null) {
				...
				if (hotbarItem.instanceId != null) {
					inventory.hotbar[index] = new Item(hotbarItem.uuid, hotbarItem.instanceId,
							inventory.nonStackableItems.get(hotbarItem.uuid).get(hotbarItem.instanceId));
				} else {
		...
		try {
			...
			for (JsonElement element : root.getAsJsonObject().get("hotbar").getAsJsonArray()) {
				...
				if (count > 0) {
					...
					String instanceId = element.getAsJsonObject().has("instance_data")
							? element.getAsJsonObject().get("instance_data").getAsJsonObject().get("id").getAsString()
							: null;
					if (instanceId != null) {
		...
		try (StringWriter stringWriter = new StringWriter();
			...
			for (Item item : inventory.hotbar) {
				...
				if (item != null && item.instanceId != null) {
					...
					jsonWriter.name("health")
							.value(((float) (itemInfo.maxWear - item.wear) / (float) itemInfo.maxWear) * 100.0f);
					jsonWriter.endObject();
				}

				jsonWriter.name("category").beginObject();
				jsonWriter.name("loc").value(
						itemInfo != null ? itemInfo.category.loc : EarthItemCatalog.ItemInfo.Category.INVALID.loc);
				jsonWriter.name("value").value(
						itemInfo != null ? itemInfo.category.value
								: EarthItemCatalog.ItemInfo.Category.INVALID.value);
				jsonWriter.endObject();
				jsonWriter.name("rarity").beginObject();
				jsonWriter.name("loc")
						.value(itemInfo != null ? itemInfo.rarity.loc
								: EarthItemCatalog.ItemInfo.Rarity.INVALID.loc);
				jsonWriter.name("value").value(
						itemInfo != null ? itemInfo.rarity.value : EarthItemCatalog.ItemInfo.Rarity.INVALID.value);
				jsonWriter.endObject();
			...
			for (String uuid : itemIds) {
				...
				if (itemInfo.stackable) {
					int count = inventory.stackableItems.getOrDefault(uuid, 0)
							- hotbarItemCounts.getOrDefault(uuid, 0);
					if (count < 0) {
					...
					HashMap<String, Integer> instances = inventory.nonStackableItems.getOrDefault(uuid,
							new HashMap<>());
					...
					if (!instanceIds.isEmpty()) {
						for (String instanceId : instanceIds) {
							...
							jsonWriter.name("health")
									.value(((float) (itemInfo.maxWear - wear) / (float) itemInfo.maxWear) * 100.0f);
							jsonWriter.endObject();
		...
		if (itemInfo.stackable) {
			try {
				...
				LogManager.getLogger().error(
						"Connector plugin threw exception when handling inventory item added {} {}", item.uuid,
						item.count, exception);
			}
		} else {
			if (item.instanceId == null) {
	...
	public int takeItem(@NotNull Item item) {
		...
		if (itemInfo.stackable) {
			try {
				...
				LogManager.getLogger().error(
						"Connector plugin threw exception when handling inventory item removed {} {}", item.uuid,
						item.count, exception);
				return 0;
			}
		} else {
			...
			try {
				return this.playerConnectorPluginWrapper.onPlayerInventoryRemoveItem(item.uuid, item.instanceId) ? 1
						: 0;
			} catch (ConnectorPlugin.ConnectorPluginException exception) {
				LogManager.getLogger().error(
						"Connector plugin threw exception when handling inventory item removed {} {}", item.uuid,
	...
	public void updateItemWear(@NotNull Item item) {
		...
		try {
			...
			LogManager.getLogger().error(
					"Connector plugin threw exception when handling inventory item wear updated {} {}", item.uuid,
					item.instanceId, exception);
		}
	}
	...
	public void setHotbar(Item[] hotbar) {
		...
		try {
			this.playerConnectorPluginWrapper.onPlayerInventorySetHotbar(Arrays.stream(hotbar)
					.map(item -> item != null && item.count > 0 ? (item.instanceId != null
							? new micheal65536.fountain.connector.plugin.Inventory.HotbarItem(item.uuid,
									item.instanceId)
							: new micheal65536.fountain.connector.plugin.Inventory.HotbarItem(item.uuid, item.count))
							: null)
					.toArray(micheal65536.fountain.connector.plugin.Inventory.HotbarItem[]::new));
		} catch (ConnectorPlugin.ConnectorPluginException exception) {
	...
	public Item toGenoaItem(@NotNull ItemStack itemStack) {
		...
		if (itemInfo.stackable) {
			...
			NbtMap customData = components != null
					? (NbtMap) components.get(DataComponentType.CUSTOM_DATA)
					: null;

			String instanceId = customData != null
					? customData.getString("GenoaInstanceId", UUID.randomUUID().toString())
					: UUID.randomUUID().toString();

			int wear = components != null
					? (Integer) components.get(DataComponentType.DAMAGE)
					: 0;
	...
	private ItemStack toItemStack(@NotNull String name, int aux, @Nullable String instanceId, int wear, int count) {
		...
		if (instanceId != null) {
			...
			components.put(DataComponentType.DAMAGE, wear);
			components.put(
					DataComponentType.CUSTOM_DATA,
					NbtMap.builder()
							.putString("GenoaInstanceId", instanceId)
							.build());

			return new ItemStack(javaId, count, components);
		} else {
